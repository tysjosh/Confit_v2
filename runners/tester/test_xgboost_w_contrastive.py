...
def tokenize_job(
    ...
    for i, job in all_job_data.iterrows():
        ...
        # added to allow augmented jobs to be used
        i_no_prefix = i.replace("aug_eda_", "").replace("aug_chatgpt_", "")
        if i_no_prefix in trainable_jids:
...
def tokenize_resume(
    ...
    for i, resume in all_resume_data.iterrows():
        ...
        # added to allow augmented resumes to be used
        i_no_prefix = i.replace("aug_eda_", "").replace("aug_chatgpt_", "")
        if i_no_prefix in trainable_rids:
...
def construct_rj_mappings(label_pairs: List[Tuple[str, str, int]],):
    r_to_j_mappping = {}
    j_to_r_mapping = {}
...
def train_DT_metric(
    ...
    n_random_neg_per_sample: int,
    n_hard_neg_per_sample: int,
):
    train_qids = 0
    ...
    r_to_j_mappping, j_to_r_mapping = construct_rj_mappings(training_labels.to_dict('records'))

    for _, row in training_labels.iterrows():
        if int(row['satisfied']) == 0:
            continue
        user_id = row['user_id']
        jd_no = row['jd_no']
        ...
        ### group 1: rank resume = given a job + multiple resume. So augment with random resumes 
        ...
        # add hard negatives
        sampled_hard_negs = j_to_r_mapping[jd_no]['neg_r']
        _n_hard_neg_per_sample = min(n_hard_neg_per_sample, len(sampled_hard_negs))
        sampled_hard_negs = np.random.choice(list(sampled_hard_negs), size=_n_hard_neg_per_sample, replace=False)
        for hard_uid in sampled_hard_negs:
            train_resume_representations.append(resume_id_to_representations[hard_uid])
            train_job_representations.append(job_id_to_representations[jd_no])
            train_labels.append(0)
            train_rank_resume_group_ids.append(__train_rank_resume_group_id_mapping[jd_no])
        # add random negatives
        samplable_keys = list(resume_id_to_representations.keys())
        samplable_keys.remove(user_id)
        for hard_uid in sampled_hard_negs:
            samplable_keys.remove(hard_uid)
        random_user_id = np.random.choice(samplable_keys, size=n_random_neg_per_sample, replace=False)
        for random_uid in random_user_id:
            train_resume_representations.append(resume_id_to_representations[random_uid])
            train_job_representations.append(job_id_to_representations[jd_no])
            train_labels.append(0)
            train_rank_resume_group_ids.append(__train_rank_resume_group_id_mapping[jd_no])

        ### group 2: rank job = given a resume + multiple job. So augment with random jobs 
        ...
        # add hard negatives
        sampled_hard_negs = r_to_j_mappping[user_id]['neg_j']
        _n_hard_neg_per_sample = min(n_hard_neg_per_sample, len(sampled_hard_negs))
        sampled_hard_negs = np.random.choice(list(sampled_hard_negs), size=_n_hard_neg_per_sample, replace=False)
        for hard_jid in sampled_hard_negs:
            train_resume_representations.append(resume_id_to_representations[user_id])
            train_job_representations.append(job_id_to_representations[hard_jid])
            train_labels.append(0)
            train_rank_job_group_ids.append(__train_rank_job_group_id_mapping[user_id])
        # add random negatives
        samplable_keys = list(job_id_to_representations.keys())
        samplable_keys.remove(jd_no)
        for hard_jid in sampled_hard_negs:
            samplable_keys.remove(hard_jid)
        random_jd_no = np.random.choice(samplable_keys, size=n_random_neg_per_sample, replace=False)
        for random_jid in random_jd_no:
            train_resume_representations.append(resume_id_to_representations[user_id])
            train_job_representations.append(job_id_to_representations[random_jid])
            train_labels.append(0)
            train_rank_job_group_ids.append(__train_rank_job_group_id_mapping[user_id])
    ...
    ranking_hparam.pop('early_stopping_rounds', None)
...
def eval_gold(
    ...
    metric, rank_job_metric, rank_resume_metric = train_DT_metric(
        ...
        resume_id_to_representations,
        job_id_to_representations,
        n_random_neg_per_sample,
        n_hard_neg_per_sample
    )
...
def load_data(
    ...
    if args.dset == 'recruiting_data':
        print('loading recruiting_data data')
        all_resume_data = pd.read_csv('dataset/recruiting_data/all_resume_eda_n_chatgpt.csv')
        all_resume_data['user_id'] = all_resume_data['user_id'].astype(str)
        all_resume_data.index = all_resume_data['user_id'].values

        all_job_data = pd.read_csv('dataset/recruiting_data/all_jd_eda_n_chatgpt.csv')
        ...
        trainable_jids = set(pd.read_csv('dataset/recruiting_data/trainable_jd.csv')['jd_no'].astype(str).values)
        trainable_rids = set(pd.read_csv('dataset/recruiting_data/trainable_resume.csv')['user_id'].astype(str).values)

        training_labels = pd.read_json('dataset/recruiting_data/train_labeled_data_eda_n_chatgpt.jsonl', lines=True)
        training_labels['user_id'] = training_labels['user_id'].astype(str)
        training_labels['jd_no'] = training_labels['jd_no'].astype(str)
        validation_labels = pd.read_json('dataset/recruiting_data/valid_classification_data.jsonl', lines=True)
        validation_labels['user_id'] = validation_labels['user_id'].astype(str)
        validation_labels['jd_no'] = validation_labels['jd_no'].astype(str)
        classification_test_labels = pd.read_json('dataset/recruiting_data/test_classification_data.jsonl', lines=True)
        ...
        rank_job_test_labels = json.load(open('dataset/recruiting_data/rank_job.json', encoding='utf-8'))
        rank_resume_test_labels = json.load(open('dataset/recruiting_data/rank_resume.json', encoding='utf-8'))
    else:
        print('loading aliyun data')
        all_resume_data = pd.read_csv('dataset/AliTianChi/all_resume_eda_n_chatgpt.csv')
        all_resume_data['user_id'] = all_resume_data['user_id'].astype(str)
        all_resume_data.index = all_resume_data['user_id'].values

        all_job_data = pd.read_csv('dataset/AliTianChi/all_jd_eda_n_chatgpt.csv')
        all_job_data['jd_no'] = all_job_data['jd_no'].astype(str)
        all_job_data.index = all_job_data['jd_no'].values
        ...
        training_labels = pd.read_json('dataset/AliTianChi/train_labeled_data_eda_n_chatgpt.jsonl', lines=True)
        training_labels['user_id'] = training_labels['user_id'].astype(str)
        training_labels['jd_no'] = training_labels['jd_no'].astype(str)
...
def main(args: argparse.Namespace):
    ...
    xgboost_perf = eval_gold(
        ...
        proprietary_jd_to_words_simple if args.dset == 'recruiting_data' else ali_jd_to_words_simple,
        proprietary_resume_to_words_simple if args.dset == 'recruiting_data' else ali_resume_to_words_simple,
        ...
        args.n_random_neg_per_sample,
        args.n_hard_neg_per_sample
    )
...
if __name__ == '__main__':
    # example: mimic ConFit training with neural nets
    # CUDA_VISIBLE_DEVICES=0 python runners/tester/test_xgboost_w_contrastive.py  \
    # --dset recruiting_data --encoder tfidf \
    # --n_random_neg_per_sample 7 \
    # --n_hard_neg_per_sample 1
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--dset', type=str, default='recruiting_data',
        choices=['recruiting_data', 'aliyun']
    )
    parser.add_argument(
        '--encoder', type=str, default='tfidf',
        choices=['bow', 'tfidf']
    )
    parser.add_argument(
        "--n_random_neg_per_sample", type=int, default=8,
        help="number of random negatives per sample. this would mimic 'batch size in contrastive learning w BERT/E5"
    )
    parser.add_argument(
        "--n_hard_neg_per_sample", type=int, default=1,
        help="number of hard negatives per sample. used in our contrastive learning configuration w BERT/E5."
    )
