...
def eval_gold(
    train_valid_resume_id_to_representations: dict,
    train_valid_job_id_to_representations: dict,
    test_resume_id_to_representations: dict,
    test_job_id_to_representations: dict,
    training_labels: pd.DataFrame,
    validation_labels: pd.DataFrame,
    classification_test_labels: pd.DataFrame,
    ...
    print('resume_id_to_representations.size', len(train_valid_resume_id_to_representations))
    for _, rep in train_valid_resume_id_to_representations.items():
        print(len(rep))
        break
    ...
    metric, rank_job_metric, rank_resume_metric = train_DT_metric(
        training_labels,
        validation_labels,
        train_valid_resume_id_to_representations,
        train_valid_job_id_to_representations,
    )

    # get scores
    classfication_score_report = evaluate_classification(
        classification_test_labels,
        test_resume_id_to_representations,
        test_job_id_to_representations,
        metric
    )
...
def load_data(
    ...
    if args.dset == 'recruiting_data':
        ...
        all_resume_data = pd.read_csv('dataset/recruiting_data/all_resume.csv')
        all_resume_data['user_id'] = all_resume_data['user_id'].astype(str)
        all_resume_data.index = all_resume_data['user_id'].values

        all_job_data = pd.read_csv('dataset/recruiting_data/all_jd.csv')
        ...
        training_labels = pd.read_json('dataset/recruiting_data/train_labeled_data.jsonl', lines=True)
...
def load_embeddings(args: argparse.Namespace):
    resume_train_valid_embedding_path = args.resume_train_valid_embedding_path
    resume_test_embedding_path = args.resume_test_embedding_path
    job_train_valid_embedding_path = args.job_train_valid_embedding_path
    job_test_embedding_path = args.job_test_embedding_path
    with jsonlines.open(resume_train_valid_embedding_path, mode='r') as reader:
        every_resume_to_train_valid = list(reader)
    with jsonlines.open(resume_test_embedding_path, mode='r') as reader:
        every_resume_to_test = list(reader)
    with jsonlines.open(job_train_valid_embedding_path, mode='r') as reader:
        every_job_to_train_valid = list(reader)
    with jsonlines.open(job_test_embedding_path, mode='r') as reader:
        every_job_to_test = list(reader)

    every_resume_to_train_valid_dict = {}
    every_job_to_train_valid_dict = {}

    for d in every_resume_to_train_valid:
        rid = d['rid']
        embedding = d['embedding']
        every_resume_to_train_valid_dict[rid] = embedding

    for d in every_job_to_train_valid:
        jid = d['jid']
        embedding = d['embedding']
        every_job_to_train_valid_dict[jid] = embedding
    ...
    for d in every_resume_to_test:
        rid = d['rid']
        embedding = d['embedding']
        every_resume_to_test_dict[rid] = embedding
        every_resume_to_train_valid_dict[rid] = embedding

    for d in every_job_to_test:
        jid = d['jid']
        embedding = d['embedding']
        every_job_to_test_dict[jid] = embedding
        every_job_to_train_valid_dict[jid] = embedding
    
    print('loaded resume train and valid', len(every_resume_to_train_valid_dict))
    print('loaded resume test', len(every_resume_to_test_dict))
    print('loaded job train and valid', len(every_job_to_train_valid_dict))
    print('loaded job test', len(every_job_to_test_dict))
    return every_resume_to_train_valid_dict, every_resume_to_test_dict, every_job_to_train_valid_dict, every_job_to_test_dict


def main(args: argparse.Namespace):
    all_data = load_data(args)
    r_train_valid_dict, r_test_dict, j_train_valid_dict, j_test_dict = load_embeddings(args)
...
if __name__ == '__main__':
    # for examples, see
    # model_checkpoints/openai/aliyun/all_resume_train_embeddings.jsonl
    # model_checkpoints/bert-base-chinese/aliyun/all_resume_train_embeddings.jsonl
    # model_checkpoints/openai/recruiting_data/all_resume_train_embeddings.jsonl
    # model_checkpoints/bert-base-multilingual-cased/recruiting_data/all_resume_train_embeddings.jsonl
    ...
    parser.add_argument(
        '--resume_train_valid_embedding_path', type=str,
        default="model_checkpoints/openai/recruiting_data/all_resume_train_embeddings.jsonl"
    )
    parser.add_argument(
        '--resume_test_embedding_path', type=str,
        default="model_checkpoints/openai/recruiting_data/all_resume_test_embeddings.jsonl"
    )
    parser.add_argument(
        '--job_train_valid_embedding_path', type=str,
        default="model_checkpoints/openai/recruiting_data/all_job_train_embeddings.jsonl"
    )
    parser.add_argument(
        '--job_test_embedding_path', type=str,
        default="model_checkpoints/openai/recruiting_data/all_job_test_embeddings.jsonl"
    )
