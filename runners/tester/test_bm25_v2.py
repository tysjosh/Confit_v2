...
def get_BM25_metric(
    ...
    bm25_rank_resume = BM25Okapi(all_resume_docs)  # seems to be better than BM25L
    test_pbar = tqdm(total=len(rank_resume_test_labels) + len(rank_job_test_labels), desc='scoring')
    for jid, data in rank_resume_test_labels.items():
    ...
    bm25_rank_job = BM25Okapi(all_job_docs)  # seems to be better than BM25L
    for rid, data in rank_job_test_labels.items():
...
def load_data(
    ...
    if args.dset == 'recruiting_data':
        ...
        all_resume_data = pd.read_csv('dataset/recruiting_data_0729/processed_recruiting_data/all_resume_full_desensitized_recruiting_data_confit_v1.csv')
        all_resume_data['user_id'] = all_resume_data['user_id'].astype(str)
        all_resume_data.index = all_resume_data['user_id'].values

        all_job_data = pd.read_csv('dataset/recruiting_data_0729/processed_recruiting_data/all_jd_full_from_text_recruiting_data_confit_v1.csv')
        all_job_data['jd_no'] = all_job_data['jd_no'].astype(str)
        all_job_data.index = all_job_data['jd_no'].values

        all_train_labels = pd.read_json('dataset/recruiting_data_0729/processed_recruiting_data/train_labeled_data.jsonl', lines=True)

        # we allow for augmented data by adding the line "i_no_prefix in trainable_rids"
        trainable_jids = set(all_train_labels['jd_no'].astype(str).values)
        trainable_rids = set(all_train_labels['user_id'].astype(str).values)

        rank_job_test_labels = json.load(open('dataset/recruiting_data_0729/processed_recruiting_data/rank_job.json', encoding='utf-8'))
        rank_resume_test_labels = json.load(open('dataset/recruiting_data_0729/processed_recruiting_data/rank_resume.json', encoding='utf-8'))
    elif args.dset == "recruiting_data-hard":
        print('loading recruiting_data Hard data')
        all_resume_data = pd.read_csv('dataset/recruiting_data_0729/processed_recruiting_data/all_resume_full_desensitized_recruiting_data_confit_v1.csv')
        all_resume_data['user_id'] = all_resume_data['user_id'].astype(str)
        all_resume_data.index = all_resume_data['user_id'].values
        ...
        rank_job_test_labels = json.load(open('dataset/recruiting_data_0729/processed_recruiting_data/rank_job_100_finegrained.json', encoding='utf-8'))
        rank_resume_test_labels = json.load(open('dataset/recruiting_data_0729/processed_recruiting_data/rank_resume_1000_finegrained.json', encoding='utf-8'))
    elif args.dset == "recruiting_data-random":
        print('loading recruiting_data random data (c.f. recruiting_data-Hard)')
        all_resume_data = pd.read_csv('dataset/recruiting_data_0729/processed_recruiting_data/all_resume_full_desensitized_recruiting_data_confit_v1.csv')
        all_resume_data['user_id'] = all_resume_data['user_id'].astype(str)
        all_resume_data.index = all_resume_data['user_id'].values
        ...
        rank_job_test_labels = json.load(open('dataset/recruiting_data_0729/processed_recruiting_data/rank_job_100_random.json', encoding='utf-8'))
        rank_resume_test_labels = json.load(open('dataset/recruiting_data_0729/processed_recruiting_data/rank_resume_1000_random.json', encoding='utf-8'))
    else:
...
def main(args: argparse.Namespace):
    ...
    bm25_perf = eval_gold(
        ...
        mp_proprietary_jd_to_words_simple if 'recruiting_data' in args.dset else mp_ali_jd_to_words_simple,
        mp_proprietary_resume_to_words_simple if 'recruiting_data' in args.dset else mp_ali_resume_to_words_simple,
        all_data['rank_job_test_labels'],
        all_data['rank_resume_test_labels'],
    )

    print(json.dumps(bm25_perf, indent=4))
    return

if __name__ == '__main__':
    # example
    # CUDA_VISIBLE_DEVICES=0 python runners/tester/test_bm25.py \
    # --dset recruiting_data
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--dset', type=str, default='recruiting_data',
        choices=['recruiting_data', 'recruiting_data-hard', 'recruiting_data-random', 'aliyun']  # 'recruiting_data-random' is to ablate recruiting_data-Hard
    )
