...
def load_data(
    ...
    if args.dset == 'recruiting_data':
        ...
        all_resume_data = pd.read_csv('dataset/recruiting_data_0514/all_resume.csv')
        all_resume_data['user_id'] = all_resume_data['user_id'].astype(str)
        all_resume_data.index = all_resume_data['user_id'].values

        all_job_data = pd.read_csv('dataset/recruiting_data_0514/all_jd.csv')
        ...
        trainable_jids = set(pd.read_csv('dataset/recruiting_data_0514/trainable_jd.csv')['jd_no'].astype(str).values)
        trainable_rids = set(pd.read_csv('dataset/recruiting_data_0514/trainable_resume.csv')['user_id'].astype(str).values)

        rank_job_test_labels = json.load(open('dataset/recruiting_data_0514/rank_job.json', encoding='utf-8'))
        rank_resume_test_labels = json.load(open('dataset/recruiting_data_0514/rank_resume.json', encoding='utf-8'))
        ...
        rank_job_test_labels = json.load(open('dataset/recruiting_data_0514/rank_job_100_finegrained.json', encoding='utf-8'))
        rank_resume_test_labels = json.load(open('dataset/recruiting_data_0514/rank_resume_1000_finegrained.json', encoding='utf-8'))
    else:
...
def main(args: argparse.Namespace):
    ...
    bm25_perf = eval_gold(
        ...
        mp_proprietary_jd_to_words_simple if args.dset == 'recruiting_data' else mp_ali_jd_to_words_simple,
        mp_proprietary_resume_to_words_simple if args.dset == 'recruiting_data' else mp_ali_resume_to_words_simple,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=['recruiting_data', 'recruiting_data-hard', 'aliyun']
    )
