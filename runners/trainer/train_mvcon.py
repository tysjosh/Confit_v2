...
from src.preprocess.dataset import RJPair2DSimplifiedDataset
...
class TrainingArguments:
    log_every_n_steps: int = field(default=10, metadata={"help": "Log every n steps."})
    save_path: str = field(
        default="model_checkpoints/InEXIT/custom",
        metadata={"help": "The output directory."},
    )
    max_epochs: int = field(default=10, metadata={"help": "Max epochs."})
    train_batch_size: int = field(default=4, metadata={"help": "Train batch size."})
    val_batch_size: int = field(default=4, metadata={"help": "Validation batch size."})
    strategy: str = field(
        default="ddp",
...
def get_dataloaders(
    model_args: MVCoNModelArguments,
    ...
    assert (
        model_args.num_resume_features == num_resume_features
    ), f"num_resume_features specified {model_args.num_resume_features} != {num_resume_features}"
    assert (
        model_args.num_job_features == num_job_features
    ), f"num_job_features specified {model_args.num_job_features} != {num_job_features}"

    # dataset
    all_resume_data = pd.read_csv(data_args.resume_data_path)
    all_job_data = pd.read_csv(data_args.job_data_path)
    training_labels: pd.DataFrame = pd.read_json(data_args.train_label_path, lines=True)
    ...
    train_dataset = RJPair2DSimplifiedDataset(
        tokenizer,
        max_seq_len=256,  # fixed for MvCoN TextCNN to work
    ...
    valid_dataset = RJPair2DSimplifiedDataset(
        tokenizer,
        max_seq_len=256,
        resume_key_names=resume_key_names,
        job_key_names=job_key_names,
...
def test_mvcon(
    model, model_args, trainer_args: TrainingArguments, data_args: DataArguments
):
    print("Testing...")
    test_args = ScoreNetworkTestArguments(
        ...
        query_prefix=data_args.query_prefix,
        batch_size=8,  # fixed as MVCoN can take large memory
    )
    ...
    (
        ...
        model, model_args, test_args, all_test_data
    )
...
def main(
    ...
    train_dataloader, valid_dataloader = get_dataloaders(
        model_args, trainer_args, data_args
    )
    ...
    set_seed(trainer_args.seed)
    test_mvcon(model, model_args, trainer_args, data_args)
    return


if __name__ == "__main__":
    parser = HfArgumentParser(
        dataclass_types=(MVCoNModelArguments, TrainingArguments, DataArguments),
