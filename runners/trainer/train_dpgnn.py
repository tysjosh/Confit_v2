...
from src.model.dpgnn import DPGNNModelArguments, DPGNNModel
from src.config.dataset import DATASET_CONFIG
from src.preprocess.dataset import RJPairNewContrastiveDataset, rj_pair_w_symm_negatives_collate_fn
from src.utils.test_score_networks import (
    ScoreNetworkTestArguments,
    load_test_data,
    get_metric_and_representations_for_dpgnn,
    evaluate,
)
...
def get_dataloaders(
    model_args: DPGNNModelArguments,
    ...
    train_dataloader = DataLoader(
        ...
        shuffle=True,
        collate_fn=rj_pair_w_symm_negatives_collate_fn,
        drop_last=True,  # unavoidable error if there is only one batch at last AND that one has no negative samples
    )
...
def test_dpgnn(model, model_args, trainer_args: TrainingArguments, data_args: DataArguments):
    print("Testing...")
...
def main(model_args: DPGNNModelArguments, trainer_args: TrainingArguments, data_args: DataArguments):
    ...
    set_seed(trainer_args.seed)
    test_dpgnn(model, model_args, trainer_args, data_args)
    return


if __name__ == "__main__":
    parser = HfArgumentParser(
		dataclass_types=(DPGNNModelArguments, TrainingArguments, DataArguments),
