...
from src.preprocess.dataset import RJPairNewContrastiveDataset, contrastive_rj_pair_w_indicies_collate_fn, RJPairHardNegContrastiveDataset
...
class DataArguments:
    ...
    confit_v1_path: str = field(
        default="",
        metadata={"help": "Path to the ConFiT v1 model."},
    )
    hard_negative_path: str = field(	
        default="",	
        metadata={"help": "Path to the hard negatives."},	
    )
    hard_negative_valid_path: str = field(
        default="",
        metadata={"help": "Path to the hard negatives for validation."},
    )
...
def get_dataloaders(
    ...
    if data_args.hard_negative_path:
        with open(data_args.hard_negative_path, 'r') as f:
            hard_negatives = json.load(f)
        train_dataset = RJPairHardNegContrastiveDataset(
            tokenizer,
            ...
            encode_all= data_args.encode_all
        )
    else:
    ...
    if data_args.hard_negative_valid_path:
        with open(data_args.hard_negative_valid_path, 'r') as f:
            valid_hard_negatives = json.load(f)
        valid_dataset = RJPairHardNegContrastiveDataset(
            tokenizer,
            max_seq_length_per_key=max_seq_len_per_feature,
            resume_key_names=resume_key_names,
            job_key_names=job_key_names,
            ...
            job_taxon_token=job_taxon_token,
            query_prefix=data_args.query_prefix,
            hard_negatives=valid_hard_negatives,
            encode_all= data_args.encode_all
        )
    else:
...
def main(model_args: ConFitModelArguments, trainer_args: TrainingArguments, data_args: DataArguments):
    ...
    data_args.encode_all = model_args.encode_all
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val/loss",#monitor="val/rank_job_ndcg@10",
        mode="min",
        save_top_k=-1, # set -1 to save every ckpt
    ...
    if data_args.confit_v1_path:
        state_dict = torch.load(data_args.confit_v1_path)["state_dict"]
    ...
    # uncomment this part of code if you want to save checkpoints of original architecture
    # if model_args.use_lora:
    #     model.bert_resume = model.bert_resume.merge_and_unload()
    
    # load best model and test
        
    print('loading best', checkpoint_callback.best_model_path)
    ...
    print('loading last', checkpoint_callback.last_model_path)
    
    
    if "deepspeed" in trainer_args.strategy:
        wrapped_state_dict = get_fp32_state_dict_from_zero_checkpoint(checkpoint_callback.last_model_path)
        state_dict = {k.replace("_forward_module.", ""): v for k, v in wrapped_state_dict.items()}

        # clean up the zero folder
        output_file = checkpoint_callback.last_model_path + ".fp32"
        print(f"Saving fp32 state dict to {output_file}")
        torch.save({'state_dict': state_dict}, output_file)
        if os.path.exists(checkpoint_callback.last_model_path) and os.path.isdir(checkpoint_callback.best_model_path):
            shutil.rmtree(checkpoint_callback.last_model_path)
        checkpoint_callback.last_model_path = output_file
    else:
        state_dict = torch.load(checkpoint_callback.last_model_path)["state_dict"]
    state_dict = torch.load(checkpoint_callback.last_model_path)["state_dict"]
    model.load_state_dict(state_dict)
    model = model.float()  # ensures no bf16 in model
    model.eval()
