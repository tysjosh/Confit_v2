...
from src.preprocess.dataset import RJPairDataset, rj_pair_collate_fn
...
class TrainingArguments:
    ...
    val_batch_size: int = field(default=8, metadata={"help": "Validation batch size."})
    log_group: str = field(
        default="aliyun",
        metadata={
    ...
    no_save: bool = field(
        default=False,
        metadata={"help": "Whether to save the best model checkpoint."}
    )
...
def get_dataloaders(
    model_args: InEXITModelArguments,
...
def main(
    ...
    set_seed(trainer_args.seed)
    test_inexit(model, model_args, trainer_args, data_args)
    return


if __name__ == "__main__":
    parser = HfArgumentParser(
        dataclass_types=(InEXITModelArguments, TrainingArguments, DataArguments),
