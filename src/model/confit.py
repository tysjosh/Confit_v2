...
class ConFitModelArguments:
    ...
    use_linear:  bool =field(
        default=False,
        metadata={"help":"whether to train with linear layer"}
    )
    encode_all: bool = field(
        default=False,
        metadata={"help": "Whether to encode all keys in resume and job"}
    )
    double: bool = field(
        default=True,
        metadata={"help": "Whether to use double sided loss"}
    )
...
def _confit_safe_2d(x):
    """Return a 2D tensor or None for empty/malformed hard-negative tensors."""
    if x is None:
        return None
...
class ConFitModel(BaseModel):
    ...
    def __init__(self, args: ConFitModelArguments):
        ...
        if args.share_encoder:
            ...
            #self.bert_resume = AutoModel.from_pretrained(args.pretrained_encoder,cache_dir="./",trust_remote_code=True)
           
            if "jina" in args.pretrained_encoder and "v2" in args.pretrained_encoder:
                try:
                    self.bert_resume = AutoModel.from_pretrained(args.pretrained_encoder,cache_dir="./",trust_remote_code=True)
                except:
                     self.bert_resume = JinaBertModel.from_pretrained("path_to_cache_folder_jinav2") # some transformer versions conflict with AutoModel loading Jina
            else:
                self.bert_resume = AutoModel.from_pretrained(args.pretrained_encoder,cache_dir="./",trust_remote_code=True)
            ...
            try:
                ...
                self.bert_resume = JinaBertModel.from_pretrained("path_to_cache_folder_jinav2")
            try:
                   
                self.bert_job = AutoModel.from_pretrained(args.pretrained_encoder,cache_dir="./",trust_remote_code=True)
            except:
                self.bert_job = JinaBertModel.from_pretrained("path_to_cache_folder_jinav2")
        ...
        ) if args.num_encoder_layers >0 else torch.nn.Identity()
        
        self.resume_linear = torch.nn.Linear(768,768) if args.use_linear else torch.nn.Identity()
        self.resume_merge_layer = torch.nn.Linear(self.args.num_resume_features, 1) if not args.encode_all else torch.nn.Identity()
        self.resume_suitablity_encoder = torch.nn.TransformerEncoder(
            ...
            num_layers=1,
        ) if not args.encode_all else torch.nn.Identity()
        ...
        self.job_linear = torch.nn.Linear(768,768) if args.use_linear else torch.nn.Identity()
        self.job_merge_layer = torch.nn.Linear(self.args.num_job_features, 1) if not args.encode_all else torch.nn.Identity()
    ...
    def forward(
        ...
        linear_layer = (
            self.resume_linear if data_type == "resume" else self.job_linear
        )
        finegrained_layer = (
        ...
        coarse_vec = linear_layer(coarse_vec)
    ...
    def compute_coarsegrained_loss_dist(
        ...
        if gathered_batched_all_coarse_job.ndim == 3:
        ...
        if gathered_batched_all_coarse_resume.ndim == 3:
    ...
    def training_step(self, batch, batch_idx) -> torch.Tensor:
        ...
        # print("resume only",neg_resume_only)
        # print("neg resume",len(batched_resume_hard_negatives))
        # print("neg job",len(batched_job_hard_negatives))
        ...
        loss_func = self.compute_coarsegrained_loss_dist if self.args.double else self.compute_coarsegrained_loss
        self.trainer.strategy.barrier()
        coarse_compute_output = loss_func(
        batched_resume_coarse_vec,
        batched_job_coarse_vec,
        batched_resume_hard_negatives,
        batched_resume_hard_negatives_indices,
        ...
        contrastive_loss_r2j = coarse_compute_output["contrastive_loss_r2j"]
        contrastive_loss_j2r = coarse_compute_output["contrastive_loss_j2r"]
        ...
        with torch.no_grad():
            # if self.trainer.is_global_zero:
                ...
                self.log("train/contrastive_loss_r2j",contrastive_loss_r2j)
                self.log("train/contrastive_loss_j2r",contrastive_loss_j2r)
