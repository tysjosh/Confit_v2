...
class RJPairSimplifiedDataset(Dataset):
    ...
    def _encode_single_dict(self, dict_data: Dict[str, str], type: str):
        ...
        encoded_dict_ = []
        if self.encode_all:
            total_field_max_len = sum([self.max_seq_length_per_key[k] for k in keys_to_encode])
            self.tokenizer_args["max_length"] = total_field_max_len
            content = ""
            for k in keys_to_encode:
                v = dict_data[k]
                content += f"{taxon_token}: section {k}:\n{v}\n"
...
class RJPairNewContrastiveDataset(Dataset):
    ...
    def _encode_single_dict(self, dict_data: Dict[str, str], type: str):
        ...
        if self.encode_all:
            total_field_max_len = sum([self.max_seq_length_per_key[k] for k in keys_to_encode])
            #print("max_length",total_field_max_len)
            self.tokenizer_args["max_length"] = total_field_max_len
            content = ""
            for k in keys_to_encode:
                v = dict_data[k]
...
class RJPairHardNegContrastiveDataset(Dataset):
    """
    encode data to the format of
    a dict resume/job would become: {
        "desired_city": {
    ...
    def __getitem__(self, index):
        ...
        # get hard negatives
        hard_negative_resume_ids = self.hard_negatives["j_to_r"][job_id]
        hard_negative_resume_ids = random.sample(hard_negative_resume_ids, min(1, len(hard_negative_resume_ids)))
        hard_negatives_resumes_encoded = []
        for user_id in hard_negative_resume_ids:
            resume = self.resume_df.loc[user_id].to_dict()
            resume.pop("user_id")
        ...
        hard_negative_job_ids = self.hard_negatives["r_to_j"][resume_id]
        hard_negative_job_ids = random.sample(hard_negative_job_ids, min(1, len(hard_negative_job_ids)))
        hard_negatives_jobs_encoded = []
        for jd_no in hard_negative_job_ids:
            job = self.job_df.loc[jd_no].to_dict()
            job.pop("jd_no")
