...
class RJPairNewContrastiveDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        hard_negative_resume_ids = []
        for i in range(len(self.resume_rank[job_id])):
            paired_score = self.resume_rank[job_id]['paired_score']
            if self.lower_bound*paired_score <= self.resume_rank[job_id]['score'][i] <= self.upper_bound*paired_score:
                hard_negative_resume_ids.append(self.resume_rank[job_id]['user_id'][i])         
        ...
        for i in range(len(self.job_rank[resume_id])):
            paired_score = self.job_rank[resume_id]['paired_score']
            if self.lower_bound*paired_score <= self.job_rank[resume_id]['score'][i] <= self.upper_bound*paired_score:
                hard_negative_job_ids.append(self.job_rank[resume_id]['jd_no'][i])
